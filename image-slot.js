...
(() => {
  const STATE_FILE = 'imageslots.state.json';
  ...
  const subs = new Set();
  let slots = {};
  // Instant hydration: a page can embed a snapshot of some/all sidecar
  // entries as <script type="application/json" data-image-slots-preload>
  // (placed before this script tag so it's already parsed). This seeds
  // `slots` synchronously — before any <image-slot> connects — so the
  // first render shows the real photo with no fetch round-trip. The
  // regular sidecar fetch below still runs after and only adds/refreshes
  ...
  const stylesheet =
    ...
    // touch-action:none only while editable — on a live (read-only) page it
    // would swallow touch scrolling anywhere a finger lands on an image.
    ...
    // Viewer mode (no write host): hide all editor chrome — dashed ring and
    // the drop-here placeholder — so live sites show clean empty frames.
    ':host(:not([data-editable])) .ring{display:none}' +
    ':host(:not([data-editable])) .empty{visibility:hidden;pointer-events:none}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Single authoritative toggle — idempotent and computed straight from
      // `url`, so data-filled can never drift out of sync with what's
      // actually shown (a prior version set/cleared this attribute from two
      // separate branches, which was fragile across re-renders).
      this.toggleAttribute('data-filled', !!url);
